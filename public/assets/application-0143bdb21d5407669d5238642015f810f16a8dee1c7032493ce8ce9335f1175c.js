    ...
    add_commit_list: function(commit_list, html_list) {
      ...
      for (i = 0, len = commit_list.length; i < len; i++) {
        ...
        results.push((function(commit) {
          var bonus_cls, bonus_symbols, btn_cls;
          bonus_cls = '';
          btn_cls = '';
          bonus_symbols = '';
  ...
  TestLogs = {
    setup: function() {
      if ($('.test-log-link').length) {
        return $('.test-log-link').each(function() {
          var anchor;
          anchor = $(this);
          return $.ajax({
            url: anchor.attr('href'),
